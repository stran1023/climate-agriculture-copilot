...
export function scoreHarness(files) {
  ...
  const checks = {
    instructions: [
      ...
      textHas(agents, ['Startup Workflow', 'Operating Loop', 'Before writing code', 'start of every session'], 'Startup workflow documented'),
      textHas(agents, ['Definition of Done', 'Completion Gate', 'done only when', 'passing only after'], 'Definition of done documented'),
    ...
    state: [
      ...
      textHas(progress, ['Current State', 'What', 'Next', 'Objective'], 'Progress log supports restart'),
    ...
    scope: [
      ...
      textHas(agents, ['Definition of Done', 'Completion Gate'], 'Completion gate limits scope closure')
    ],
    lifecycle: [
      hasFile(byPath, ['init.sh'], 'Startup script exists'),
      textHas(agents, ['End of Session', 'Before ending', 'Before You Stop'], 'End-of-session procedure exists'),
...
function jsonFeatureList(text, message) {
  try {
    ...
    const valid = Array.isArray(parsed.features) && parsed.features.every((feature) =>
      ...
      && (typeof feature.name === 'string' || typeof feature.title === 'string')
      && (typeof feature.description === 'string' || typeof feature.user_visible_behavior === 'string')
...
export function htmlReport(result, title = 'Harness Assessment', extraSections = '') {
