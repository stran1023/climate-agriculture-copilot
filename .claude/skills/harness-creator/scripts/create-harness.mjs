...
const agentTemplate = agentFile === 'CLAUDE.md' ? 'CLAUDE.md' : 'AGENTS.md';
const replacements = {
...
const results = [];
results.push(await copyTemplate(agentTemplate, path.join(target, agentFile), replacements, { force }));
results.push(await copyTemplate('feature_list.json', path.join(target, 'feature_list.json'), {}, { force }));
